...
--   array + elements.record → structured list editor
--   map                     → key-value editor (CXR → Radyoloji - ...)
--   record                  → nested form (array of single-key tables)
--   string                  → text input (Kong OSS'ta multi-line/textarea YOK)
--   number                  → numeric input
--   boolean                 → checkbox
--
-- NOT: Tüm alanlar TOP-LEVEL `fields` array'inde olmalı. Record içine
-- sarılırsa (örn. `config = { type = "record", fields = {...} }`) Kong
-- Manager generic field renderer'a düşürüyor ve `set` alanları bile
-- text-input olarak gösteriyor. Bu yüzden 26 alan düz (flat) listede.
--
-- 26 toplam alan:
...
--   Inner record `fields` (örn. category_size_hints.values, phi_patterns.elements)
--   da ARRAY olmalı, named-key map değil.
-- ==========================================================================
...
return {
  ...
  fields = {
    ...
    -- 1. HTTP methods (multi-select chips — protocols gibi)
    { allowed_methods = {
        type = "set",
        elements = {
          type = "string",
    ...
    -- 5. Allowed medical categories (multi-select chips)
    { allowed_categories = {
        type = "set",
        elements = {
          type = "string",
    ...
    -- 15. System prompt template (long string, placeholders runtime'da değişir)
    --     Kong OSS'ta textarea yok, bu tek satır input olarak render edilir.
    ...
    -- 18. Enable image_tokens calculation
    ...
    -- 19. Jailbreak patterns (EN). set + string → tag input.
    { jailbreak_patterns = {
        type = "set",
    ...
    -- 20. Turkish injection patterns (tag input)
    ...
    -- 21. Medical safety patterns (EN). tag input.
    { medical_safety_en = {
        type = "set",
        elements = { type = "string" },
        default = {
    ...
    -- 22. Medical safety patterns (TR). tag input.
    { medical_safety_tr = {
        type = "set",
        elements = { type = "string" },
        default = {
    ...
    -- 23. System prompt extraction patterns. tag input.
    ...
    -- 24. Output sanitization patterns (XSS / HTML / JS / template). tag input.
    ...
    -- 25. PHI/PII regex patterns (KVKK / GDPR). array of record.
    { phi_patterns = {
        type = "array",
        elements = {
          type = "record",
